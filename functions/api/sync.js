...
async function readDoc(env, key){
  ...
  return { items: Array.isArray(res.items) ? res.items : [], version: Number.isFinite(res.version) ? res.version : 0, updatedAt: res.updatedAt || new Date().toISOString() };
}
...
export const onRequestPost = async ({ request, env }) => {
  ...
  const key = `household:${id}`;
  let client;
  try { client = await request.json(); } catch (e) { return bad(400, "invalid json"); }
  ...
  for (const ci of clientItems) {
    ...
    if (!si || cLU >= sLU) mergedMap.set(ci.id, ci);
  }
